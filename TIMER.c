...
uint8 Running_Flag[MAX_NUM_OF_TIMERS]={0,0,0};
...
ACK Time_Delay ( TIMER_t TIMER_Select , double Required_Delay  )
{
	...
	PORTA |= (1<<PA4);
	//PORTC=0xFF;
	Required_Delay = (Required_Delay) * (1000);  //seven zeros
	...
	OCR_Value = OCR_Value/1000;					//seven zeros
	//PORTC=OCR_Value;
/*************************************************************************/
...
ACK PWM_generate( TIMER_t TIMER_Select , float Required_duty_cycle , uint8 type )
{
	...
	if(1)
	{
		switch(TIMER_Select)
		{
			...
			{
				PORTA |= (1<<PA0);
				OCR0 = (255*(Required_duty_cycle/100));
				break;
			}
			...
			{
				Top=65535;
				OCR1A= (Top*(Required_duty_cycle/100));
				break;
			}
			...
			{
				Top=255;
				OCR2 = (Top*(Required_duty_cycle/100));
				break;
			}
